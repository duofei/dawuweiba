<?php echo CHtml::beginForm(url('admin/shop/categoryCreate'),'post',array('name'=>'add'));?>
...
<table  class="tabcolor list-tbl ma-b5px ma-t5px" width="100%" cellspacing="1">
  ...
  <?php foreach ($category as $key=>$val):?>
    <tr <?php if ($key%2 != 0) { echo 'class="divbg1"'; } ?>>
	    ...
	    <td>
	    <?php if($val->state == 1){ ?>
	    	<a href="<?php echo url('admin/shop/categoryEdit', array('id'=>$val->id))?>"><span class="color">编辑</span></a>&nbsp;&nbsp;
	    	<a href="<?php echo url('admin/shop/categoryDelete', array('id'=>$val->id))?>" onclick="return confirm('确定要禁用吗？');"><span class="color">禁用</span></a>
	    <?php }?>
	    </td>
    </tr>
  <?php endforeach;?>
