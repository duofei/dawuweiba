<div id="yw0" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
    <ul class="ui-tabs-nav ui-helper-reset ui-helper-clearfix ui-widget-header">
	  ...
	  <li class="ui-state-default ui-corner-top ui-tabs-selected ui-state-active"><a href="<?php echo url("shopcp/order/finish");?>">已完成订单</a></li>
...
<table  class="tabcolor list-tbl ma-b5px" width="100%">
  ...
  <table class="list-tbl bline order-title" width="100%">
	  <tr>
	    ...
	    <td width="60">&yen;<?php echo $val->actualMoney?></td>
	    <td width="50"><?php echo $val->deliveryMan->name?></td>
	    <td width="60">
	    	<?php if($val->user_id > 0) :?>
		    <?php echo $val->userCreditLogs->evaluatesText?>
		    <?php if ($val->userCreditLogs->evaluate == null) {?>
		    <a href="<?php echo url('shopcp/order/evaluate', array(evaluate=>UserCreditLog::EVALUATE_GOOD, id=>$val->id, user_id=>$val->user_id))?>"><span class="color">好评</span></a>
		    <a href="<?php echo url('shopcp/order/evaluate', array(evaluate=>UserCreditLog::EVALUATE_BAD, id=>$val->id, user_id=>$val->user_id))?>"><span class="color">差评</span></a>
		    <?php }?>
		    <?php endif;?>
	    </td>
	  </tr>
