<h2 class="pa-l10px cred f16px bline lh30px">我要提交地址</h2>

<?php echo CHtml::beginForm(url('location/create'), 'post');?>
<div class="m10px f14px">
	<div>如果发现您搜索的地址不存在，请您告诉我们，谢谢！</div>
	<?php if($success):?>
	<h2 class="ma-t20px ma-b20px lh30px f20px"><?php echo $success;?></h2>
	<?php else:?>
	<div class="ma-t10px">
		地址名称：<?php echo CHtml::activeTextField($location, 'name', array('class' => 'txt'));?>
	</div>
	<div class="ma-t10px">
		详细地址：<?php echo CHtml::activeTextField($location, 'address', array('class' => 'txt'));?>
	</div>
	<div class="ma-t10px">
		<?php echo CHtml::activeHiddenField($location, 'map_x');?>
		<?php echo CHtml::activeHiddenField($location, 'map_y');?>
		地图标注：<?php echo l('开始标注', 'javascript:void(0);', array('onclick'=>'openDialog()'));?>
		<span id="showMapXY"><?php echo $location->map_x;?>, <?php echo $location->map_y;?></span>
	</div>
	<div class="ma-t10px">
	验证　码：
	<?php echo CHtml::activetextField($location ,'validateCode',  array('class'=>'validate-code fnum fb txt f16px', 'tabindex'=>3, 'maxlength'=>4))?>
