...
<div class="fl ma-r10px w720px" >
 ...
 <div class="group-sort f14px">
	...
	<?php foreach ((array)$category as $key=>$val) :?>
		<li><a href="<?php echo aurl('tuannav/secondSearch', array('category_id' => $val->id, 'sort'=>'sell'))?>" title="<?php echo $val->name?>"><?php echo $val->name?></a></li>
	<?php endforeach;?>
    </ul>
 ...
 <div class="ma-t10px border-red pa-b10px h-100" >
    ...
    <div class="pa-10px">
		<div class="border-bottom-dot pa-5px ma-b5px">
            <ul class="subfl">
                <li class="sort bg-red pa-5px ma-r10px" >转让</li>
                <li class="sort pa-5px"><a href="<?php echo url('tuannav/second', array('sort'=>'buy'))?>">求购</a></li>
            </ul><div class="clear"></div>
       </div>
	    ...
	    <div class="border-bottom-dot pa-5px">
	         ...
	         <?php else :?>
	                <li class="sort pa-5px ma-r10px <?php if (!$_GET['category_id']) echo 'bg-red';?>"><a href="<?php echo url('tuannav/second', array('sort'=>'sell'))?>">全部</a></li>
	         <?php endif;?>
	         <?php foreach ((array)$category as $key=>$val) :?>
	         		<?php if ($val->id == $_GET['category_id']): ?>
	            		<li class="ma-r10px pa-5px bg-red"><?php echo $val->name?></li>
	            	<?php else:?>
	            		<li class="ma-r10px pa-5px"><a href="<?php echo aurl('tuannav/secondSearch', array('category_id' => $val->id, 'sort'=>'sell'))?>"><?php echo $val->name?></a></li>
		            <?php endif;?>
	         <?php endforeach;?>
	            </ul>
	        
	        <div class="fr ma-r40px  f14px cwhite">
	        <a class=" block ico-post-four lh30px fb ac" href="<?php echo url('tuannav/secondCreate')?>">发布转让</a>
	        </div><div class="clear"></div>
	    </div>
    ...
    <?php if ($secondhard):?>
        ...
        <table class=" seller-tab"  width="100%">
          <tr class="cblack">
            <td width="80%">
            <div class="fl <?php if ($val->state == STATE_ENABLED) echo 'a-bargain'; else echo 'a-seller';?> ico-w35px "></div>
